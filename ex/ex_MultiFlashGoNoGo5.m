function result = ex_MultiFlashGoNoGo5(e)
...
if isfield(e,'dot1Color')
    ...
    if e.stimNum == 1
        ...
        msg('set 7 oval 0 %i %i %i %i %i %i',[newX newY e.fixRad*30 e.dot1Color(1) e.dot1Color(2) e.dot1Color(3)]);
        msg('set 8 oval 0 %i %i %i %i %i %i',[newX newY e.fixRad*30 targ(1) targ(2) targ(3)]);
    elseif e.stimNum == 2
        targs = reshape(e.dot2Targs,[3,length(e.dot2Targs)/3]);
        targ = targs(:,e.numTarg);
        msg('set 7 oval 0 %i %i %i %i %i %i',[newX newY e.fixRad*30 e.dot2Color(1) e.dot2Color(2) e.dot2Color(3)]);
        msg('set 8 oval 0 %i %i %i %i %i %i',[newX newY e.fixRad*30 targ(1) targ(2) targ(3)]);
    end
