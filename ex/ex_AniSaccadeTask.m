function result = ex_AniSaccadeTask(e)
...
%
% 2015/08/14 by Matt Smith - added ACQUIRE_TARG code before sacc_stayOnTarget
% window
%
...
%
    ...
    if behav.prevTrial == 1 % previous trial exists ANI 12/2/24, figure out next x/y 
        nexttheta = deg2rad(behav.newAngle);
        nextX = round(e.distance*cos(nexttheta));
        nextY = round(e.distance*sin(nexttheta));
    ...
    % of the edge of the screen
        ...
        if (abs(nextX) + e.sacc_size > (params.displayWidth/2 - extraborder))
            %disp('X exceeds limit, moving fix pt');
            shiftX = abs(nextX) + e.sacc_size - params.displayWidth/2 + extraborder;
            if nextX > 0
        ...
        if (abs(nextY) + e.sacc_size > (params.displayHeight/2 - extraborder))
            %disp('Y exceeds limit, moving fix pt');
            shiftY = abs(nextY) + e.sacc_size - params.displayHeight/2 + extraborder;
            if nextY > 0
        ...
        msg('set 2 oval 0 %i %i %i %i %i %i',[nextX nextY e.sacc_size e.targetColor(1) e.targetColor(2) e.targetColor(3)]);
        if isfield(e,'helperTargetColor')
            msg('set 3 oval 0 %i %i %i %i %i %i',[nextX nextY e.sacc_size e.helperTargetColor(1) e.helperTargetColor(2) e.helperTargetColor(3)]);
        end
    ...
    %     msgAndWait('ack'); %commented out 03Apr2013, seemed to be causing problems....
        ...
        if ~waitForFixation(e.saccadeTime,nextX,nextY,targetWindowRadius)
            % didn't reach target
            sendCode(codes.NO_CHOICE);
            msgAndWait('all_off');
            sendCode(codes.FIX_OFF);
        ...
        if ~waitForMS(e.sacc_stayOnTarget,nextX,nextY,targetWindowRadius)
        ...
        result = 1;
        behav.prevTrial = 1 ;
        behav.newAngle = behav.newAngle + e.nextAngle;
    ...
    %     msgAndWait('ack'); %commented out 03Apr2013, seemed to be causing problems....
        ...
        result = 1;
        behav.prevTrial = 1 ;
        behav.newAngle = e.angle + e.nextAngle;
