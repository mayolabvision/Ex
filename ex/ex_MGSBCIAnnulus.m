function result = ex_DistanceBCI(e)
...
% angle
    ...
    if ~isfield(behav,'bcicorrect')
        ...
        behav(end).dimblock = behav(end-1).dimblock;
        behav(end).dimtrialind = behav(end-1).dimtrialind;
        behav(end).misscount = behav(end-1).misscount;
        behav(end).remindcount = behav(end-1).remindcount;
        behav(end).autoFlipSaccTaskCount = behav(end-1).autoFlipSaccTaskCount;
        if isfield(e,'iterativeRecal') && e.iterativeRecal==1
    ...
    switch mod(behav(end).trialNum,e.miniblocksize),
        case 0, %indicates start of new miniblock
            ...
            if behav(end).trialNum>0&&~behav(end).flipFlag, %don't flip before the first miniblock
                ...
                if e.miniblocksize > 1
                    ...
                    behav(end).cue = e.angle(randi(length(e.angle),1));
                end
                ...
                behav(end).dimblock = e.dimblock;
                if e.randomDimFlag == 1
                    behav(end).dimtrialind = randi(e.miniblocksize-(e.dimCueStartTrial-1),1)+(e.dimCueStartTrial-1);
                end
            end;
        ...
        otherwise
            behav(end).flipFlag = 0;
            if e.autoFlipSaccTaskFlag == 1 && behav(end).autoFlipSaccTaskCount >= e.autoFlipSaccTaskflip
                behav(end).flipFlag = 1;
                behav(end).autoFlipSaccTaskCount = 0;
                behav(end).trialNum = 0;
                if length(behav(end).superBlockCues)==length(e.angle)
                    behav(end).superBlockCues = [];
                end
                diff = setdiff(e.angle,behav(end).cue); %set the current cue to the "other" one
                thisPerm = randperm(length(diff),1);
                if e.miniblocksize > 1
                    behav(end).cue = diff(thisPerm);
                else
    ...
    if mod(behav(end).trialNum,e.miniblocksize)<e.nCueTrials
        ...
        targOnFlag = 1;
        cueTrialFlag = 1;
    elseif e.leaveTargetOnFlag == 1
        targOnFlag = 1;
    elseif e.flagCountMissTargOn == 1 && behav(end).misscount >= e.numMissCue 
        targOnFlag = 1;
        behav(end).autoFlipSaccTaskCount = 0;
        if behav(end).remindcount >= e.numRemindTrials
            behav(end).misscount = 0 ;
            behav(end).remindcount = 0;
            targOnFlag = 0;
            behav(end).autoFlipSaccTaskCount = 0;
        end
    end;
    remind = behav(end).remindcount;
    display(remind)
    miss = behav(end).misscount;
    display(miss)
    display(targOnFlag)
    ...
    if (mod(behav(end).trialNum,e.miniblocksize)+1)>=e.dimCueStartTrial && behav(end).dimblock==1
        e.targetColor = e.dimtargetColor;
    elseif e.randomDimFlag == 1 && (mod(behav(end).trialNum,e.miniblocksize)+1) == behav(end).dimtrialind
        e.targetColor = e.dimtargetColor;
    end
    
    if e.showDelayTargetFlag == 1
        if cueTrialFlag == 1 
            targetColorOther = e.targetColorOtherCue;
        else
            if ~isfield(e,'targetColorOtherBrightness')
                targetColorOther = e.targetColorOther;
            else
                targetColorOther = e.targetColorOtherBrightness(e.targetColorOtherInd)*ones(size(e.targetColorOther));
            end
        end
    else
        targetColorOther = e.bgColor;
    end
    sendStruct(struct('angle',behav(end).cue,'bciTrial',e.bciTrial,'superBlock',behav(end).superBlockCues,'trialNum',behav(end).trialNum,'isSham',isSham,'targetColor',e.targetColor,'dimblock',behav(end).dimblock,'targetColorOther',targetColorOther,'targOnFlag',targOnFlag));
    result = 0;
...
%         disp(['set ' num2str(objID) ' ' runString]);
   ...
   % msg('set 4 oval 0 %i %i %i %i %i %i',[0 0 e.cursorSize e.cursorColor(1) e.cursorColor(2) e.cursorColor(3)]);
    ...
    if  e.bciTrial == 1 || e.annulusvisibleflag == 1
        ...
        msg('set %i oval 0 %i %i %i %i %i %i',[4 e.fixX e.fixY (innerrad) e.bgColor(1) e.bgColor(2) e.bgColor(3)]);
        msg('set %i oval 0 %i %i %i %i %i %i',[5 e.fixX e.fixY (outerrad) e.bgColor(1) e.bgColor(2) e.bgColor(3)]);
    end
    minnewobj = 6;
    numcolors = 100;
...
%         end     
        ...
        [fixsuccess, bcisuccess, behav(end).cursorloc,timevec] = waitForMSmatlabUDP_MGSBCIannulus(waitRemainder,e.fixX,e.fixY,params.fixWinRad,newX,newY,find(e.angle == e.delayAngle),bciTargRad,bcifun,...
...
%         end
            ...
            if e.bciTrial == 1
                if bcirewardflag == 1
                    ...
                    if isfield(e,'iterativeRecal') && e.iterativeRecal==1
                        ...
                        if (behav(end).currRecal<=length(e.recalTrial)) && (totalbcitrials==e.recalTrial(behav(end).currRecal))
    ...
    if isfield(e,'fixandMGSFlag')&&e.fixandMGSFlag==1
        ...
        if waitForMS(e.saccadeInitiate,e.fixX,e.fixY,newFixWinRad,'recenterFlag',params.recenterFixWin)
            ...
            matlabUDP2('send',sockets(2),'trialend');
            if targOnFlag == 0
        ...
        if targOnFlag == 0 % this needed to allow for multiple reminder trials in a row
            behav(end).misscount = 0; % this makes it so needs N in a row incorrect
            behav(end).remindcount = 0;
            behav(end).autoFlipSaccTaskCount = behav(end).autoFlipSaccTaskCount + 1;
        else
            behav(end).remindcount = behav(end).remindcount + 1;
        end
        if e.bciTrial == 0 || e.bciRewardEnable==0 || e.bciSaccadeJuice == 1
            sendCode(codes.REWARD);
