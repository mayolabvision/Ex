function result = ex_SaccadeTask_audio(e)
...
%     msgAndWait('ack'); %commented out 03Apr2013, seemed to be causing problems....
    ...
    elseif ((e.targetOnsetDelay + e.targetDuration) < e.fixDuration)
        ...
        %preSoundTime and soundDuration could be randomized if necessary
       
        if ~waitForMS(e.preSoundTime,e.fixX,e.fixY,params.fixWinRad)
            % didn't hold fixation during the period after target offset &
            % before sound onset
            sendCode(codes.BROKE_FIX);
            msgAndWait('all_off');
            sendCode(codes.FIX_OFF);
        ...
        if e.playSound>0 %if > 0, then this trial has a sound
            playTone(e.soundfreq,e.soundDuration,1,0,0,0);
            sendCode(codes.SOUND_ON)
            if ~waitForMS(e.soundDuration,e.fixX,e.fixY,params.fixWinRad)
                sendCode(codes.BROKE_FIX);
                msgAndWait('all_off');
        ...
        %remaining wait in trial after "sound period" = extra delay
        waitRemainder = e.fixDuration - (e.targetOnsetDelay + e.targetDuration + e.preSoundTime + e.soundDuration);
